...
class RelativePositionBias2D(nn.Module):
    ...
    def forward(self, coords_2d: torch.Tensor):
        """coords_2d: (B, N, 2) normalized [0,1] patch coordinates."""
...
class TopoFlowAttention(nn.Module):
    ...
    def _compute_elevation_bias(self, elevation_patches):
        """Penalize uphill attention: -alpha * ReLU(elev_diff)."""
    ...
    def forward(self, x, coords_2d, elevation_patches):
        ...
        Args:
            ...
            coords_2d: (B, N, 2) normalized patch coordinates
            elevation_patches: (B, N) mean elevation per patch
        """
        B, N, D = x.shape
        ...
        # (q@k^T) * scale → (q*scale) @ k^T keeps intermediate values small
...
class TopoFlowBlock(nn.Module):
    """Pre-norm transformer block with TopoFlow attention (elevation + position bias)."""
...
def compute_patch_coords(img_size, patch_size, device):
    """Compute normalized [0,1] coordinates for each patch center."""
    H, W = img_size
...
def compute_patch_elevations(elevation_field, patch_size):
    """Average-pool elevation to get per-patch elevation.

    Args:
        elevation_field: (B, H, W) elevation map
        patch_size: int or (ph, pw)
    Returns:
        (B, num_patches) mean elevation per patch
    """
